...
export default function ImageUploader({ value = [], onChange, max = 12, folder = 'projects', className = '' }) {
  ...
  return (
    <div className={className}>
      ...
      {editingIndex >= 0 && value[editingIndex] && (
        <div className="mt-4 rounded-3xl border border-gold/40 bg-cream p-4">
      ...
      {value.length > 0 && (
        <p className="mt-3 text-xs text-ink/45">
          Drag images to reorder. The first image is used as the cover.
        </p>
      )}
    </div>
  );
